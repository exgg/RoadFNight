...
namespace Opsive.UltimateCharacterController.AddOns.Multiplayer.PhotonPun.Game
{
    ...
    public abstract class SpawnManagerBase : MonoBehaviourPunCallbacks, IOnEventCallback
    {
        ...
        private ResizableArray<PhotonView> m_Players = new ResizableArray<PhotonView>();
        private SendOptions m_ReliableSendOption;
        private RaiseEventOptions m_RaiseEventOptions;
        private Dictionary<int, PhotonView> m_ActorNumberByPhotonView = new Dictionary<int, PhotonView>();
        ...
        private struct InactivePlayer
        {
            ...
            public InactivePlayer(Vector3 position, Quaternion rotation, ScheduledEventBase removeEvent)
            {
                Position = position;
                Rotation = rotation;
        ...
        private void AddPhotonView(PhotonView photonView)
        {
            ...
            m_ActorNumberByPhotonView.Add(photonView.OwnerActorNr, photonView);
        }
        ...
        public override void OnPlayerLeftRoom(Player otherPlayer)
        {
            ...
            if (otherPlayer != null && m_ActorNumberByPhotonView.TryGetValue(otherPlayer.ActorNumber, out var photonView)) {
                ...
                if (otherPlayer != null) {
                    if (otherPlayer.IsInactive) {
                        ...
                        m_InactivePlayers.Add(otherPlayer, new InactivePlayer(photonView.transform.position, photonView.transform.rotation, removeEvent));
                    }

                    EventHandler.ExecuteEvent("OnPlayerLeftRoom", otherPlayer, photonView.gameObject);
                    m_ActorNumberByPhotonView.Remove(otherPlayer.ActorNumber);
                }
